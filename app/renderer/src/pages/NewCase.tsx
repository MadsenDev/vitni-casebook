...
export function NewCase() {
  ...
  const { createCase, openCase } = useAppStore()
  ...
  const handleCreate = async () => {
    ...
    const newCase = await createCase(caseName.trim())
      ...
      <div style={{ overflow: 'auto', display: 'flex', flexDirection: 'column', alignItems: 'center', padding: '48px 32px' }}>
        ...
        <div style={{ width: '100%', maxWidth: 720 }}>
          ...
          <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginTop: 32 }}>
            ...
            <button onClick={() => step < 3 ? setStep(s => s + 1) : handleCreate()} style={{ padding: '9px 28px', background: P.stamp, color: P.mode === 'dark' ? '#1c1812' : '#f3ebd8', border: 'none', borderRadius: 3, fontFamily: "'Newsreader', serif", fontStyle: 'italic', fontSize: 15, fontWeight: 600, cursor: 'pointer' }}>{step < 3 ? 'Continue →' : 'Create case'}</button>
          </div>
        </div>
      </div>
