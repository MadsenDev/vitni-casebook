  ...
  return (
    <div style={{ height: '100%', overflow: 'auto', padding: '28px 30px', background: P.paperRaised, borderLeft: `1px solid ${P.rule}`, fontFamily: "'Newsreader', 'Source Serif 4', Georgia, serif", color: P.ink }}>
      ...
      <div style={{ marginTop: 14, display: 'grid', gridTemplateColumns: 'auto 1fr', gap: '8px 14px', fontSize: 13 }}>
        {([['Subject', item.node?.label ?? '—'], ['Type', item.node?.type ?? '—'], ['Author', a.by], ['Created', a.at], ['Confidence', `${Math.round(a.confidence * 100)}% (${a.sources.length} src)`], ['Status', decision ?? a.status]] as [string, string][]).map(([k, v]) => (
...
export function Review({ onSearch, onAvatar }: { onSearch?: () => void; onAvatar?: () => void }) {
  const { P } = useTheme()
  const { nodes, assertions: assertionsMap } = useAppStore()

  // Build review queue from store: all assertions with status 'review'
  const queue: QueueItem[] = Object.entries(assertionsMap).flatMap(([nodeId, list]) => {
    const node = nodes.find(n => n.id === nodeId)
    if (!node) return []
    return list
      .filter(a => a.status === 'review' || a.status === 'disputed')
      .map(a => ({
        node,
        aId: a.id,
        priority: a.confidence < 0.5 ? 'high' as const : a.confidence < 0.75 ? 'med' as const : 'low' as const,
        age: a.updatedAt.slice(0, 10),
        evidence: a.sources[0] ?? '—',
        thumb: 'doc' as const,
        assertion: { id: a.id, text: a.text, sources: a.sources, status: a.status as AssertionStatus, confidence: a.confidence, by: a.byUser, at: a.updatedAt.slice(0, 10) },
      }))
  })

  const [selected, setSelected] = useState<string>(queue[0]?.assertion.id ?? '')
  ...
  const queues = [{ id: 'to-review', label: 'To review', count: queue.length }, { id: 'disputed', label: 'Disputed', count: 0 }, { id: 'changed', label: 'Recently changed', count: 0 }, { id: 'promoted', label: 'Promoted today', count: 0 }, { id: 'rejected', label: 'Rejected', count: 0 }]
  ...
  return (
    <div style={{ width: '100%', height: '100vh', background: P.paper, color: P.ink, fontFamily: "'Newsreader', 'Source Serif 4', Georgia, serif", display: 'grid', gridTemplateColumns: '232px 1fr 420px', gridTemplateRows: '64px 1fr 56px', gridTemplateAreas: "'top top top' 'left main right' 'bot bot bot'", overflow: 'hidden' }}>
      ...
      <div style={{ gridArea: 'right', overflow: 'hidden' }}>
        {item
          ? <Provenance item={item} decision={decisions[item.assertion.id]} P={P} />
          : <div style={{ height: '100%', background: P.paperRaised, borderLeft: `1px solid ${P.rule}`, display: 'flex', alignItems: 'center', justifyContent: 'center', color: P.inkMuted, fontStyle: 'italic', fontSize: 14, fontFamily: "'Newsreader', serif" }}>Nothing to review</div>
        }
      </div>
      <div style={{ gridArea: 'bot', background: P.paperSunk, borderTop: `1px solid ${P.rule}`, padding: '0 22px', display: 'flex', alignItems: 'center', gap: 18, fontFamily: "'JetBrains Mono', monospace", fontSize: 10, color: P.inkMuted, letterSpacing: '0.12em' }}>
        ...
        <div style={{ flex: 1, height: 6, background: P.paper, borderRadius: 1, border: `1px solid ${P.ruleSoft}`, position: 'relative' }}>
          {queue.length > 0 && <div style={{ position: 'absolute', inset: 0, width: `${(decided / queue.length) * 100}%`, background: P.asserted, borderRadius: 1 }} />}
        </div>
        <span>{queue.length > 0 ? Math.round((decided / queue.length) * 100) : 0}%</span>
