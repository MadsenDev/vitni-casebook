...
export function CommandPalette({ onClose }: Props) {
  ...
  const { nodes, edges, assertions } = useAppStore()
  ...
  const previewNode = previewItem?.label ? nodes.find(n => n.label.toLowerCase().includes(previewItem.label.toLowerCase().split(' ')[0])) : null
    ...
    <div onClick={onClose} style={{ position: 'fixed', inset: 0, zIndex: 100, background: P.mode === 'dark' ? 'rgba(0,0,0,0.55)' : 'rgba(28,24,18,0.22)', backdropFilter: 'blur(4px)', display: 'flex', alignItems: 'flex-start', justifyContent: 'center', paddingTop: 120, fontFamily: "'Newsreader', 'Source Serif 4', Georgia, serif" }}>
      <div onClick={e => e.stopPropagation()} style={{ width: 880, background: P.paperRaised, border: `1px solid ${P.rule}`, borderRadius: 5, boxShadow: P.shadowLg, display: 'grid', gridTemplateColumns: '1fr 280px', overflow: 'hidden', maxHeight: '70vh' }}>
        ...
        <div style={{ padding: '20px 18px', background: P.paperSunk, overflow: 'auto' }}>
          {previewNode ? (
            <>
              <div style={{ fontFamily: "'JetBrains Mono', monospace", fontSize: 9.5, color: P.inkMuted, letterSpacing: '0.18em', marginBottom: 8 }}>{previewNode.type.toUpperCase()} · {previewNode.id.slice(0, 8).toUpperCase()}</div>
              ...
              <div style={{ fontFamily: "'JetBrains Mono', monospace", fontSize: 10, color: P.inkMuted, letterSpacing: '0.12em', marginBottom: 8 }}>{(assertions[previewNode.id] ?? []).length} FACTS · {edges.filter(e => e.aId === previewNode.id || e.bId === previewNode.id).length} LINKS</div>
              {(assertions[previewNode.id] ?? []).slice(0, 2).map(a => (
