...
function EventSheet({ ev, P }: { ev: TimelineEvent; P: ReturnType<typeof useTheme>['P'] }) {
  const actors = ev.actors.map(id => C.findNode(id)).filter(Boolean)
  return (
          ...
          <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '8px 18px' }}>
            {actors.map(n => n && (
              <div key={n.id} style={{ display: 'flex', gap: 10, alignItems: 'baseline', padding: '6px 0', borderBottom: `1px dotted ${P.ruleSoft}` }}>
                ...
                <span style={{ marginLeft: 'auto', fontFamily: "'JetBrains Mono', monospace", fontSize: 9.5, color: P.inkMuted, letterSpacing: '0.08em' }}>{C.typeMeta[n.type].label.toUpperCase()}</span>
              </div>
            ))}
          </div>
        </div>
