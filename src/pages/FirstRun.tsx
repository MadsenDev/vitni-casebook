...
export function FirstRun() {
  ...
  const cards = [
    ...
    { kind: 'OPEN · DISK', title: 'Open an existing case', body: 'A .casebook archive on disk. Vitni opens it offline; you choose whether to unlock now.', cta: 'Open from file…', primary: false, kbd: '⌘ O', action: () => navigate('/investigation') },
  ]
      ...
      <div style={{ overflow: 'auto', padding: '54px 80px 60px', display: 'grid', gridTemplateColumns: 'minmax(0,1fr) 380px', gap: 56, alignContent: 'start' }}>
        ...
        <aside style={{ display: 'flex', flexDirection: 'column', gap: 26 }}>
          ...
          <div style={{ padding: '20px', background: P.paperRaised, border: `1px solid ${P.rule}`, borderRadius: 4, boxShadow: P.shadow }}>
            <div style={{ fontFamily: "'JetBrains Mono', monospace", fontSize: 9.5, color: P.inkMuted, letterSpacing: '0.18em', marginBottom: 8 }}>SAMPLE CASE</div>
            <div style={{ fontSize: 17, fontStyle: 'italic', fontWeight: 600, marginBottom: 6 }}>Operation Glass Harbor</div>
            <div style={{ fontSize: 13, color: P.inkSoft, lineHeight: 1.5, marginBottom: 14 }}>Layered wire transfer · shell companies · beneficial ownership dispute. A fully worked example with sources, assertions, and a timeline.</div>
            <button onClick={() => navigate('/investigation')} style={{ width: '100%', padding: '9px 14px', background: 'transparent', color: P.ink, border: `1px solid ${P.rule}`, borderRadius: 3, cursor: 'pointer', fontFamily: "'Newsreader', serif", fontStyle: 'italic', fontSize: 13, fontWeight: 600 }}>Open sample case →</button>
          </div>
